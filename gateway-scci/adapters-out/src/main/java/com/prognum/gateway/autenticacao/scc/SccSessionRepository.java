...
public class SccSessionRepository implements SessaoPersistente {
    ...
    @Override
    public java.util.List<com.prognum.gateway.autenticacao.port.out.SessaoAtiva> listar(String ambiente) {
        SccDbConfig c = env.ler(ambiente);
        String sql = "SELECT SESSION_KEY, NO_USUARIO, NU_IP_ACESSO, DT_HORA_SOLICITACAO FROM SCCI_SESSION";
        java.util.List<com.prognum.gateway.autenticacao.port.out.SessaoAtiva> out = new java.util.ArrayList<>();
        try (Connection conn = connections.abrir(c);
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                Timestamp t = rs.getTimestamp("DT_HORA_SOLICITACAO");
                out.add(new com.prognum.gateway.autenticacao.port.out.SessaoAtiva(
