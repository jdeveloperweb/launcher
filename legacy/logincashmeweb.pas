...
function ExecutaLoginCashmeWeb(var Usuario, Senha, Login, NomePerfil : AnsiString): Boolean;
var
  Qry : TSQLQuery;
  CodPerfil : Integer;
begin
  ...
  try
    ...
    if pos('@', Usuario) > 0 then begin
      ...
      Qry.sql.add('SELECT USUARIO, PERF_PRIMARIO FROM USUARIO WHERE UPPER(USUARIO)=:usuario');
      ...
      //CPF
      while length(usuario) < 14 do
        usuario := '0' + usuario;
      Qry.sql.add('SELECT CO_CONTRATO FROM CADMUT WHERE CAD_CPF=:cpf');
      Qry.paramByName('cpf').asString := Usuario;
      Qry.open;
      if not Qry.eof then begin
        ...
        raise Exception.Create('Não existem contratos para o CPF acessado.');
    end;
  finally
    Qry.free;
  end;
  ...
  // usuario e senha
      ...
      if (trim(Usuario) <> '') then begin
        if OpStr = 'LOGIN' then begin
          ...
          if ExecutaLoginCashmeWeb(Usuario,Senha,Login,Perfil) then begin
