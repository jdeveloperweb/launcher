...
begin
  ...
  try
    ...
    Query.sql.add('SELECT COD_PERFIL FROM PERFIL WHERE COD_PERFIL='+quotedstr(perfil));
    Query.open;
...
begin
  if Entidade > '' then begin
    ...
    try
      Query.sqlconnection := GetSqlConnection(pegaDirTab);
      Query.sql.add('SELECT COD_ENT FROM ENTIDADES WHERE cod_ent='+intstr(valint(entidade)));
      Query.open;
      result := not Query.isempty;
      if result then
...
function ObtemEntidadePorEntidadeLogin(CO_ENTIDADELOGIN : string) : integer;
var
  Query : TSqlQuery;
begin
  Query := TSqlQuery.create(nil);
  try
    Query.sqlconnection := GetSqlConnection(pegaDirTab);
    Query.sql.add('SELECT CO_ENTIDADE FROM ENTIDADE_SCCI WHERE CO_ENTIDADELOGIN='+quotedstr(CO_ENTIDADELOGIN));
    Query.open;
...
procedure ParseSenha(    senha : ansistring;
                     Var EntidadePrimaria,
                         EntidadeSecundaria,
                         Perfil,
                         Setor,
                         Nome : ansistring);
var
  ...
  PhysicalDeliveryOfficeName : ansistring;
  Memberof : ansistring;
  cooperativa : ansistring;
  CO_ENTIDADELOGIN : ansistring;
  PA : ansistring;
begin
  EntidadePrimaria := '';
  EntidadeSecundaria := '';
  ...
  try
    Parametros.commatext := copy(senha,6,length(senha));
    Memberof := Parametros.values['memberof'];
    nome := Parametros.values['Givename'];
    PhysicalDeliveryOfficeName := Parametros.values['PhysicalDeliveryOfficeName'];

    cooperativa := copy(Memberof,1,4);
    Setor := '1';
    Perfil := rightStr(Memberof,3);
    
    if comparetext('PA',copy(PhysicalDeliveryOfficeName,1,2)) = 0 then begin

      // usuários de PA

      PA := trim(copy(PhysicalDeliveryOfficeName,3,length(PhysicalDeliveryOfficeName)));
      PA := stringofchar('0',5-length(PA)) + PA;
      CO_ENTIDADELOGIN := rightstr(cooperativa,2) + PA;
      EntidadePrimaria := intstr(ObtemEntidadePorEntidadeLogin(CO_ENTIDADELOGIN));

      CO_ENTIDADELOGIN := rightstr(cooperativa,2) + stringofchar('0',5);
      EntidadeSecundaria := intstr(ObtemEntidadePorEntidadeLogin(CO_ENTIDADELOGIN));
      
    end
    else begin

      // usuários de cooperativa

      CO_ENTIDADELOGIN := rightstr(cooperativa,2) + stringofchar('0',5);
      EntidadePrimaria := intstr(ObtemEntidadePorEntidadeLogin(CO_ENTIDADELOGIN));
    end;
    
    if EntidadePrimaria = '-1' then
      EntidadePrimaria := '';
    if EntidadeSecundaria = '-1' then
      EntidadeSecundaria := '';
...
function ExecutaLoginAilos  (    Usuario : ansistring;
                             Var Senha : ansistring;
                             Var Expirada : boolean) : boolean;
var
  Query : TSqlQuery;
  EntidadePrimariaUsuario,
  EntidadeSecundariaUsuario : integer;
  PerfilUsuario : integer;
  st : string;
  parmEntidadePrimaria,
  parmEntidadeSecundaria,
  parmPerfil,
  parmNome,
  parmSetor : ansistring;
begin
  result := false;
  EntidadePrimariaUsuario := -1;
  EntidadeSecundariaUsuario := -1;
  PerfilUsuario := -1;
  ParmEntidadePrimaria := '';
  ParmEntidadeSecundaria := '';
  ParmPerfil := '';
  ...
  if copy(st,1,5) = '!#___' then begin
    ParseSenha(st,ParmEntidadePrimaria,ParmEntidadeSecundaria,
               ParmPerfil,ParmSetor,ParmNome);
    if trim(ParmNome) = '' then
      ParmNome := usuario;
    if ValidaEntidade(ParmEntidadeSecundaria,EntidadeSecundariaUsuario) then
      if ValidaEntidade(ParmEntidadePrimaria,EntidadePrimariaUsuario) then
        if ValidaPerfil(parmPerfil,PerfilUsuario) then begin
          Query := TSqlQuery.Create(nil);
          try
            ...
            if Query.Eof then begin
              ...
              Query.sql.add('(USUARIO,PERF_PRIMARIO,ENT_PRIMARIA,ENT_SECUNDARIA,ALT_USUARIO,ALT_DATA,SENHA,CO_SETOR,NOME,UIDUSUARIO)');
              ...
              Query.parambyname('ENT_SECUNDARIA').datatype := ftinteger;
              ...
              Query.parambyname('ENT_PRIMARIA').asinteger := entidadePrimariaUsuario;
              if EntidadeSecundariaUsuario > -1 then
                Query.parambyname('ENT_SECUNDARIA').asinteger := entidadeSecundariaUsuario
              else
                Query.parambyname('ENT_SECUNDARIA').clear; // nulo
              ...
              Query.parambyname('CO_SETOR').asstring := ParmSetor;
              Query.parambyname('NOME').asstring := ParmNome;
        ...
        raise exception.create('Entidade primária do usuário inexistente')
    else
      raise exception.create('Entidade secundária do usuário inexistente');
  end
  else
...
{
assign(debug,'/tmp/debug1.txt');
if fileexists('/tmp/debug1.txt') then
  append(debug)
else
  rewrite(debug);
try
      ...
      begin
        ...
        begin
          if usuario = 'usuarioweb' then begin
            ...
            if ExecutaLoginAilos(usuario,Senha,Expirada) then
            begin
