...
uses {$IFDEF FPC} cthreads, {$ENDIF}SysUtils, lib1, pdb, scciio, xdb, parmlib, uloglib, punix, pxmllib, fpjwt, fpjwasha256,
     HTTPSend, Classes, pIniFiles, pcrypt, synautil, strutils, DateUtils, unix, aelib, ssl_openssl, ssl_openssl11, ssl_openssl3;
...
function ExecutaLoginSisbr(var Senha, Usuario : AnsiString; CPF : AnsiString): boolean;
var
  Qry : TSQLQuery;
  st  : AnsiString;
begin
  ...
  if copy(st,1,5) = '!#___' then begin
    ...
    try
      ...
      Qry.SqlConnection := GetSqlConnection(PegaDirTab);
      Qry.sql.Add('SELECT USUARIO FROM USUARIO WHERE UPPER(USUARIO) = '+QuotedStr(UpStr(Usuario)));
      Qry.Open;
      if Qry.Eof then begin
        Qry.close;
        Qry.sql.clear;
        if Length(CPF) < 14 then CPF := Const_Str('0', 14-Length(CPF)) + CPF;
        Qry.sql.Add('SELECT USUARIO FROM USUARIO WHERE CPF = '+QuotedStr(CPF));
        Qry.Open;
        if Qry.Eof then 
        ...
        Qry.next;
        if not Qry.Eof then
          raise exception.create('Existem 2 ou mais usuários ativos com o mesmo CPF, regularize o cadastro para prosseguir.');
      end
      else
        Usuario := Qry.FieldByName('USUARIO').AsString;
      result := true;
  ...
  // usuario e senha
      ...
      if (trim(Usuario) <> '') then begin
        if OpStr = 'LOGIN' then begin
          ...
          try
            if ExecutaLoginSisbr(Senha,Usuario,CPF) then begin
