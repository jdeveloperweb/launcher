...
echo.
echo [1/5] Compilando (mvn -DskipTests clean package)...
call mvn -q -f "%ROOT%\pom.xml" -DskipTests clean package
if errorlevel 1 goto :erro

set "JAR="
for %%f in ("%BOOT%\target\scci-core*.jar") do set "JAR=%%~ff"
...
echo [3/5] Montando pasta dist (jar + config + run.sh + JDK)...
...
echo  Pacote:        %TGZ%
echo  (scci-core na porta 8090, interno)
echo.
